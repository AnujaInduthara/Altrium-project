...
// requesting browser's Origin, so the copied link matches the host the HR user
// is actually on (localhost / 127.0.0.1 / LAN IP) with no configuration.
function withPublicUrl(vacancy, baseUrl) {
  return {
    ...vacancy,
    public_url: vacancy.public_token
      ? buildPublicApplyUrl(vacancy.public_token, baseUrl)
      : null,
  };
}
...
async function createVacancy(req, res) {
  ...
  try {
    ...
    return successResponse(res, withPublicUrl(vacancy, req.get('origin')), 201);
  } catch (err) {
...
async function listVacancies(req, res) {
  try {
    ...
    return successResponse(res, {
      vacancies: vacancies.map((vacancy) => withPublicUrl(vacancy, origin)),
    });
  } catch (err) {
